...
import mockGameFeatures from "@/data/mock-data-landing"
  ...
  return (
    <section className="w-full max-w-5xl mx-auto mb-16 bg-blue-600/50 rounded-3xl p-8 backdrop-blur-sm border-2 border-blue-400/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {mockGameFeatures.map((fish) => (
          <FeatureCards
            key={fish.id}
            title={fish.name}
            description={`Rarity: ${fish.nftMetadata.rarity}, Gen: ${fish.nftMetadata.generation}`}
            icon="🐟"
          />
        ))}
      </div>
